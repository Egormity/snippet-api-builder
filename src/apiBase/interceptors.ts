...
import { utilApiUpdateAuthTokens, utilLocalStorage, utilReplaceToAuth } from "../utils";
...
axiosWithAuth.interceptors.response.use(
	...
	(error: AxiosError) => {
		return new Promise((_, reject) => {
			if (!error.response || !error.config || !(error.response.data instanceof Blob)) reject(error);
			const reader = new FileReader();
			reader.onload = () => {
				error.response?.data && (error.response.data = JSON.parse(reader?.result?.toString() || ""));
				reject(error);
			};
			reader.onerror = () => {
				reject(error);
...
axiosWithAuth.interceptors.response.use(
	...
	async (error: AxiosError) => {
		...
		if (originalRequest.url?.includes("/auth/refresh-token")) {
			console.log("[axiosWithAuth]: Ошибка на refresh-token, переход в авторизацию");
			utilReplaceToAuth();
			throw error;
		}
		...
		if (originalRequest.headers) originalRequest.headers.Authorization = `Bearer ${newToken?.accessToken}`;
