...
export const utilHandleServiceError = ({
	...
	if (error instanceof AxiosError) {
		const errors = (error.response?.data?.errors || error.response?.data) as TErrors;
		const message = (() => {
			if (Array.isArray(errors)) return errors?.map(item => item?.errorMessage)?.join(". ");
		...
		if (
			isToast &&
			finalMessage &&
			(showToastOnAuthError || (error.status && error.status !== 401 && error.status !== 403))
		)
			alert(finalMessage);
	...
	//
	isToast && alert(baseErrorMessage);
