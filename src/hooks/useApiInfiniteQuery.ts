...
import { NullUndefinedAble, RecursiveNullUndefinedPartialAble, TMethodParams } from "../types";

import { utilApiBuildQueryParams, utilGetAppInfo } from "../utils";
	...
	const result = useInfiniteQuery<AxiosResponse<TResponse<TDataArrayItem>>, Error>({
		...
		queryFn: async ({ pageParam = 1 }: { pageParam: unknown }) => {
			try {
				...
				//
				const filtersQuery = utilApiBuildQueryParams(thisFilter);
